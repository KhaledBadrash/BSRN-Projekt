...
def lade_woerter(woerter_pfad, xachse, yachse):
    ...
    try:
        with open(woerter_pfad, 'r', encoding='utf-8') as file:  #Öffne die Wörterdatei
            ...
            if len(woerter) < anz_woerter:  #prüft, ob genügend Wörter vorhanden sind
...
def log_game_start(host_name, max_spieler):
    ...
    # Erstellt eine JSON mit folgenden Schlüsseln und Werten (Schlüssel : Wert)
    start_data = {
        'host_name': host_name,
...
def pruefe_bingo(max_feld, logs):  # Überprüft auf Bingo
    # Speichert die markierten Felder und derren Positionen:
    marked_positions = [(log.get('x_wert'), log.get('y_wert')) for log in logs
                        if log.get('button_text') == '---' or log.get('JOKER') == '---']

    # Überprüft horizontale Linien. i steht für X-Achse j Y-Achse
    for i in range(max_feld):
        if all((i, j) in marked_positions for j in range(max_feld)):
            return True

    # Überprüft vertikale Linien. i steht für X-Achse j Y-Achse
    for j in range(max_feld):
        if all((i, j) in marked_positions for i in range(max_feld)):
            return True

    # Überprüft diagonale Linien (von links oben nach rechts unten). i übernimmt beide Achsen
    if all((i, i) in marked_positions for i in range(max_feld)):
        return True

    # Überprüft diagonale Linien (von rechts oben nach links unten). i übernimmt beide Achsen
...
def winner_screen(parent, personal_name):
    ...
    # Animation des Titels mit verschiedenen Farben und Schreibarten
    def animate_title():
        #Speicherung der Textänderungen in einem Array
        ...
        index = 0  # Um die Indexes im Array zu durchlaufen
        while True:
            # Verändert das Aussehen der Titel
            ...
            index += 1
            time.sleep(0.1)  #Gibt Animationsdauer an
            parent.update()

    # Starte die Animation in einem separaten Thread, um die GUI reaktionsfähig zu halten/ müsste man aber nicht so machen
...
class GameApp:
    #  Konstruktor der Klasse GameApp
    def __init__(self, args, player_name=None):
        #  self repräsentiert die Instanzen der Klasse
        self.args = args
    ...
    def run(self):
        #  gibt in der GUI die Anordnung der Widgets/Buttons an
        grid_layout = ttk.TTkGridLayout(parent=self.root)
        self.root.setLayout(grid_layout)

        #  erstellt die Bingokarte
        for i in range(self.args.xachse):  # Element i repräsentiert die Zeilenposition
    ...
    def button_click(self, button, x_wert, y_wert):  #  Methode zur Markierung u. Zurücksetzung der Felder
        ...
        if button.text() == "---":
            ...
            button.setText("---")  # Markiert das Feld
            auswahl_zeitpunkt = datetime.now().strftime('%d-%m-%Y %H:%M:%S Uhr')  # Loggt den Zeitpunkt der Markierung
        ...
        # Überprüft nach jedem Anklicken, ob ein Bingo entstanden ist
        if pruefe_bingo(self.args.xachse, logs):
            game_over = True  # Beendet das Spiel
            winner_screen(self.root, self.player_name)  # Öffnet Gewinner-Fenster

    # Dokumentiert den Joker u. Dessen Position
    def log_joker(self, button_text, x_wert, y_wert, auswahl_zeitpunkt):
        if isinstance(button_text, ttk.TTkString):
            button_text = str(button_text)  # Konvertiert den Text im Button (in dem Fall '---') in eine String
